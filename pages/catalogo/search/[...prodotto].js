...
export async function getServerSideProps(context) {
    ...
    /** Dati prodotto */
    const { prodotto } = context.params
    const prodottoPath = prodotto[0]
    const pageNumber = prodotto[1]
    const linkPage = pageNumber ? `${prodottoPath}/${pageNumber}/` : `${prodottoPath}/`;

    /** Link interno alla pagina */
    const page = await API.get(`/${lang}/catalogo/search/${linkPage}`).then((response) => {
...
const CatalogoSearch = ({ lang, page }) => {
